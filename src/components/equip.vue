<template>
  <div class="w-player-equip" :class="{ showPosition: showPosition }">
    ...
    <div class="w-player-equip-item" :class="`equip${item.UcPos}`" v-for="item in equips" :key="item.UcPos">
      <el-popover trigger="hover" :placement="item.UcPos | placement(showPosition)">
        <div class="w-player-equip-item-detail">
          ...
          <div class="u-line u-title">
            <span class="u-name" :style="{ color: showFontColor(item.Color) }">{{ item.Name }}</span>
            <span class="u-star">
              <span v-for="i in ~~item.StrengthLevel" :key="i">★</span>
            </span>
            <span class="u-strength">精炼：{{ item.StrengthLevel }}/{{ item.MaxStrengthLevel }}</span>
          </div>
          <!-- 装本基本属性外防内防 -->
          <div class="u-line u-base">
            <span v-if="!item.UcPos">{{ item.Base1Type.Attrib.GeneratedBase }}~ {{ item.Base1Type.Base1Max | BaseMax(item.Base2Type.Base2Max) }}</span>
          ...
          <div class="u-line u-info">
            <span v-for="c in item.ModifyType" :key="c.Desc">
              <span v-if="c.Attrib.GeneratedMagic">
                ...
                <b v-if="item.StrengthLevel > 0">(+{{ c.Param1Max | refine(item.StrengthLevel) }})</b>
              </span>
          ...
          <div class="u-line u-stone u-superstone" v-if="item.ColorStone">
            ...
            <div class="u-list">
              <div v-for="c in item.ColorStone.Attributes" :key="c.Desc">{{ c.Attrib.GeneratedMagic }}</div>
            </div>
          </div>
          <!-- 等级+耐久度 -->
          <div class="u-line u-misc">
            <span>需要等级{{ item.Level }}</span>
            <span v-if="~~item.MaxDurability">耐久度：{{ item.WDurability }}/{{ item.MaxDurability }}</span>
          </div>
          <!-- 小附魔 -->
          ...
          <div class="u-line u-set" v-if="item.Set">
            ...
            <span v-for="(c, i) in item.SetList" :key="i" class="u-setname" :class="c | setwquip(setlist)">{{ c }}</span>
          ...
          <!-- 使用效果 -->
          <div class="u-line u-desc" v-if="item.Desc" :class="{ useable: item.UcPos }">{{ item.Desc | filterText }}</div>
        ...
        <div class="w-player-equip-item-icon" slot="reference">
          ...
          <span class="u-name" v-if="showEquipName" :style="{ color: showFontColor(item.Color) }">{{ item.Name }}</span>
        </div>
      </el-popover>
    </div>
  </div>
