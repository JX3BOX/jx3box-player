<template>
  <div class="m-user-equip" :class="equipUserImg">
    <div class="m-item" v-for="item in data" :key="item.UcPos">
      <el-popover :placement="item.UcPos | placement(equipUserImg)" trigger="hover">
        <div class="m-item-box">
          ...
          <!-- 装本基本属性 -->
          <div class="u-line">
            <span v-if="item.UcPos == '0'">{{ item.Base1Type.Attrib.GeneratedBase }}~ {{ item.Base1Type.Base1Max | BaseMax(item.Base2Type.Base2Max) }}</span>
        ...
        <div slot="reference" :class="`equip${item.UcPos}`" class="m-img">
          <i :class="item.Quality | quality"><img class="u-img" :src="item.Icon.FileName" alt=""/></i>
          <span v-if="equipUserImg == ''">{{ item.Name }}</span>
        </div>
      </el-popover>
    </div>
  </div>
